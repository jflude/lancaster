...
var (
	...
	env                 = "test"
	run                 = true
	wireProtocolVersion = fmt.Sprintf("%d.%d", C.version_get_wire_major(), C.version_get_wire_minor())
	execPath            = getExecDir()
...
func init() {
	...
	flag.StringVar(&wireProtocolVersion, "protocol", wireProtocolVersion, "Required wire protocol version (* means accept any)")
