...
func (p *Print) String() string {
	...
	return fmt.Sprintf("%-32s %-15s %9d %6d x %-6s %-9d %s",
		p.key(), t.Format("15:04:05.999999"), p.opraSeq, p.lastSize,
		fwfloat(p.price()), p.totalExchVolume, latency)
}
...
func (s *Summary) String() string {
	...
	return fmt.Sprintf("%-32s %-15s %9d  o:%-6.2f  h:%-6.2f  l:%-6.2f  c:%-6.2f  %9d flags:%-4d",
		s.key(), t.Format("15:04:05.999999"), s.opraSeq,
		s.open(), s.high(), s.low(), s.close(), s.totalVolume, s.flags)
}
